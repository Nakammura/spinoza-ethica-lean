...
A22 is logically weaker than A12 in our framework: A12 together
with A14 (substance has at least one attribute) entails A22, but
A22 does not entail A12, as the counter-model in §5.4 witnesses.
The asymmetry — A12 → A22 holds, A22 → A12 fails — is what
makes A22 a meaningful demote candidate. If A12 followed from
...
machine-checked confirmation against the specific Della Rocca
PSR-substance reconstruction. The confirmation is bounded: it
applies to A22 alone, plus the base axioms of `Pars1Axioms`. It
does not apply to alternative PSR commitments (Della Rocca's
"thoroughgoing PSR" remains unrefuted, and indeed unspecified
in Della Rocca 2008's prose); it does not apply to alternative
reconstructions (Garrett's strong-Definition-III route is a
different demote candidate, not yet attempted).
